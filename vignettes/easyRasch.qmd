...
This is an introduction to using the [`easyRasch` R package](https://github.com/pgmj/easyRasch). A changelog for package updates is available [here](https://github.com/pgmj/easyRasch/blob/main/NEWS.md).

::: {.callout-note icon="true"}
## Note
This package was previously known as `RISEkbmRasch`
:::

Details on package installation are available at the [package GitHub page](https://github.com/pgmj/easyRasch).
...
There is a separate GitHub repository containing a suggested template to simplify using the `easyRasch` package when conducting a reproducible Rasch analysis in R: <https://github.com/pgmj/RISEraschTemplate>
...
::: {.callout-note icon="true"}
## Note
Quarto automatically adds links to R packages and functions throughout this document. However, this feature only works properly for packages available on [CRAN](https://cran.r-project.org/). Since the `easyRasch` package is not available on CRAN the links related to functions starting with **RI** will not work.
:::
...
df.all <- read_csv("anvari2022data.csv") # or this line if you have downloaded the datafile in the /vignettes folder
...
Similarly to item fit, we need to run simulations to get a useful cutoff threshold value for when residual correlations amongst item pairs are too large to show model fit [@christensen2017].
